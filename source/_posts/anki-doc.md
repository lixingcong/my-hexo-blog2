...
注意：Reschedule不会改变某张卡片的回答历史(revision history)。历史是指：用户点击Good、Again的次数,历史会参与数据统计。Reschedule仅仅改变了卡片的状态(已毕业的卡片变成新卡片)，如果确实需要重新重置某卡片的回答历史，则需要导出改笔记，然后删掉该笔记，重新导入。
